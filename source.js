...
function setNoResults() {
  ...
  //DISPLAY RESULT BLOCK
  resultsLista.style.display = "flex";resultsLista.style.height = "fit-content";
...
//ONKEYUP SEARCH PROFILES FUNCTION MAIN
    ...
    setList(
      accounts
        ...
        .sort((personA, personB) => {
          return (
            getRelevancy(personB.username, value) - getRelevancy(personA.username, value)
          );
        })
    );
  } else {
  ...
  async getData() {
    try {
      ...
      post = post.map((item) => {
        ...
        return {username,profileimage,postcontent,likes,commentContent,mention,mentiontag,hashatags,time,idp,id,hrt,ikedbySign,and,incl,likescountrt,likeTag,hashatagsurl,likeinfo};
      });
      return post;
    } catch (error) {
      console.log(error);
